...
@section('content-new')
    ...
    <div class="content-wrapper">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title">Cpt Code RVU</h4>
{{--                <div class="row">--}}
...
{{--                                        {!! Form::select('practice', count($practices)?$practices->pluck('name', 'id'):'',null,['class' => 'form-control', 'required' => 'required']) !!}--}}
                        ...
                        <div class="table-responsive tableFixHead pagination" data-pagecount=10">
                            <table id="order-listing" class="table">
                                <thead>
                                <tr class="table-color">
                                    <th><strong> #</strong></th>
                                    <th class="table-height"><strong>CPT Code</strong> </th>
                                    <th class="table-height1"><strong>Description</strong></th>
                                    <th><strong> workRVU</strong></th>
                                    <th><strong> practiceRVU</strong></th>
                                    <th><strong> malpracticeRVU</strong></th>
                                    <th><strong> totalRVU</strong></th>
                                ...
                                @if(isset($cptCodes) && !is_null($cptCodes))
                                    @foreach($cptCodes as $cptCode)
                                        <tr class="text-white billingindex{{ !is_null($cptCode->id) ? $cptCode->id : '' }}">
                                            ...
                                            <td ><span class="editable" data-id="{{ !is_null($cptCode->id) ? $cptCode->id : '' }}" data-cptId="{{ !is_null($cptCode->id) ? $cptCode->id : '' }}"> {{ !is_null($cptCode->description)? $cptCode->description : ''   }}</span></td>

                                            <td>{{ !is_null($cptCode->workRVU)? $cptCode->workRVU : ''   }}</td>
                                            <td>{{ !is_null($cptCode->practiceRVU)? $cptCode->practiceRVU : ''   }}</td>
                                            <td>{{ !is_null($cptCode->malpracticeRVU)? $cptCode->malpracticeRVU : ''   }}</td>
                                            <td>{{ !is_null($cptCode->totalRVU)? $cptCode->totalRVU : ''   }}</td>
    ...
    <script>
        ...
        $(document).on("click", ".DeleteCpt", function() {
            if (confirm('Are you sure you want to delete?')) {
                var id = $(this).data('id');
                var url = '{!! url('admin/cptcodervu')!!}'+"/"+id;
                $.ajaxSetup({
                    headers: {
