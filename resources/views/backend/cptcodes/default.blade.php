...
@section('content-new')
    ...
    <div class="content-wrapper">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title">Cpt Code Default</h4>
                <div class="row">
                    <div class="col-md-12 text-right" style="margin-top: -50px">
                        <button type="button" class="btn btn-primary practice-btn commit-button">Add Default Cpt Code <i class="fas fa-user-plus" aria-hidden="true"></i></button>
                                ...
                                @if(count($cptCodes))
                                    @foreach($cptCodes as $cptCode)
                                        <tr class="text-white billingindex{{ !is_null($cptCode->id) ? $cptCode->id : '' }}">
                                            ...
                                            <td class="editable" data-id="{{ !is_null($cptCode->id) ? $cptCode->id : '' }}" data-cptId="{{ !is_null($cptCode->id) ? $cptCode->id : '' }}"> {{ !is_null($cptCode->par_amount)? $cptCode->par_amount : ''   }}</td>
        ...
        <div class="modal-dialog">
            <div class="modal-content pop-up">
                <div class="modal-header">
                    <h4 class="pt-3"><strong>CPT Code Default</strong></h4>
    ...
    <script>
        ...
        $(document).on("click", ".commit-button", function() {
            ...
            $.ajax({
                ...
                beforeSend: function() {
                    $('.commit-button').html('Add Cpt Code Default <i class="fas fa-cog fa-spin" aria-hidden="true"></i>');
                },
                type: 'GET',
                url: "{{ url('admin/get-cptcode-default-modal') }}",
                headers: {
                    'X-CSRF-Token': "{{csrf_token()}}"
                },
                success: function(response) {
