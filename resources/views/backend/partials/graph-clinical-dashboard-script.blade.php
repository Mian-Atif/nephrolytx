<script>
    var ctx = document.getElementById("Chart1C");
    var myChart = new Chart(ctx, {
        type: 'horizontalBar',
        data: {
            labels: ["18.6", "18.6-24.9", "25.0-29.0", "30.0 & Above"],
            datasets: [{
                label: '',
                data: [
                    {{$CKDPatientsBMI->below_186}},
                    {{$CKDPatientsBMI->rng_186_249}},
                    {{$CKDPatientsBMI->rng_250_299}},
                    {{$CKDPatientsBMI->equal_above_300}}
                ],
        ...
        onClick: function (evt, item) {
                ...
                var sideLabel = this.data.labels[activePoint._index];

            var type = '';
            if(sideLabel == '18.6'){
                type = 'bmi_below_practice_id86';
            }else if(sideLabel == '18.6-24.9'){
                type = 'bmi_rng_practice_id86_249';
            }else if(sideLabel == '25.0-29.0'){
                type = 'bmi_rng_250_299';
            }else if(sideLabel == '30.0 & Above'){
                type = 'bmi_equal_above_300';
            }

            let url = "/admin/patients-list/"+type;
...
<script>
    ...
    var BarChartGroupedNew = new Chart(document.getElementById("bar-chart-grouped-new"), {
        type: 'bar',
        data: {
            labels: [
                    'Early-Stage CKD', 'Late-Stage CKD', 'ESRD',
            ],
            datasets: [
                {
                    label: "This Year",
        ...
        options: {
            ...
            onClick:
                ...
                var type = '';
                if(label == 'This Year' && monthYear == 'Early-Stage CKD'){
                    type = 'early_this_year';
                }else if(label == 'Prior Year' && monthYear == 'Early-Stage CKD'){
                    type = 'early_prior_year';
                }else if(label == 'This Year' && monthYear == 'Late-Stage CKD'){
                    type = 'late_this_year';
                }else if(label == 'Prior Year' && monthYear == 'Late-Stage CKD'){
                    type = 'late_prior_year';
                }else if(label == 'This Year' && monthYear == 'ESRD'){
                    type = 'esrd_this_year';
                }else if(label == 'Prior Year' && monthYear == 'ESRD'){
                    type = 'esrd_prior_year';
                }
                ...
                yAxes: [
                    {
                        ticks: {
                            ...
                            // min: 0,
                            beginAtZero: true,
                            max: {{$CKDPatientsComparison->maxCDKPC}},
...
<script>
    ...
    $("#clinicalFilter #provider,#clinicalFilter #location,#clinicalFilter #payer").on("change", function(){
        alert($(this).val());
        $("#clinicalFilter").submit();
    });

    $('#clinicalFilter').submit(function (e) {
        e.preventDefault();
        var $f = $(this);
        var $sb = $f.find('[type="submit"]');
        var oldBtnText = $sb.html();
