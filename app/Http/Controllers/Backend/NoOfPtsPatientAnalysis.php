...
class
NoOfPtsPatientAnalysis extends Controller
{
...
//        $revenues=   array($ckdRevenue,$nonCkdRevenue,$esrdRevenue);
        ...
        $numberOfPatientsPerMonthPTS = $analysisController->numberOfPatientsPerMonth();
       
        $totalNumberOfPatientsPerProviderPerMonthPTS = $analysisController->TotalNumberOfPatientsPerProviderPerMonth();
    
        $numberOfPatientsByWhoPaidBillsPerMonthPTS = $analysisController->NumberOfPatientsByWhoPaidBillsPerMonth();
        
        $newpatientscountactualmonth =  DB::select('CALL new_patients_count_actual_month(?,?,?,?)', [$practice_id, $location, $provider, $insurance]);
     
        // $ckdvisitintervaltargetachieved =  DB::select('CALL ckd_visit_interval_target_achieved(?,?,?,?)', [$practice_id, $location, $provider, $insurance]);
        $ckdvisitintervaltargetachieved =  DB::select('CALL patient_dashboard_target_achieved_month_wise(?,?,?,?)', [$practice_id, $location, $provider, $insurance]);


return view('backend.no-of-pts-patientAnalysis', compact('newPatientAnalysisLabeles',
         'newPatientAnalysisTargets',
         'ckdvisitintervaltargetachieved',
         'numberOfPatientsByWhoPaidBillsPerMonthPTS',
         'newpatientscountactualmonth',
         'totalNumberOfPatientsPerProviderPerMonthPTS',
         'numberOfPatientsPerMonthPTS',
         'analysisController',
