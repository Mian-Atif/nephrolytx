...
class
PracticeDashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $patientVolume = DB::select("CALL revenue_forecast_on_patient_volume_new($practice_id,'','','','')");
        ...
        // $accountReceivables = DB::select("CALL account_receivable_collectable_openbalance_CKD_wise($practice_id,'','','')");

        $account_receivable_openbalance = DB::select("CALL account_receivable_openbalance($practice_id,'','','','')");
        $accountReceivablesOpenBalancesEsrd = Arr::pluck($account_receivable_openbalance, 'Esrd_OpenBalance');
        $accountReceivablesOpenBalancesNonEsrd = Arr::pluck($account_receivable_openbalance, 'NonEsrd_OpenBalance');
        $accountReceivablesOpenBalancesTotal = Arr::pluck($account_receivable_openbalance, 'Total_OpenBalance');
        // dd($accountReceivablesOpenBalancesEsrd,$accountReceivablesOpenBalancesNonEsrd,$accountReceivablesOpenBalancesTotal);
        $account_receivables_collectables = DB::select("CALL account_receivable_collectables($practice_id,'','','','')");
// dd($account_receivables_collectables);
         $accountReceivablesCollectablesEsrd = Arr::pluck($account_receivables_collectables, 'Esrd_Collectables');
        $accountReceivablesCollectablesNonEsrd = Arr::pluck($account_receivables_collectables, 'NonEsrd_Collectables');
        $accountReceivablesCollectablesTotal = Arr::pluck($account_receivables_collectables, 'Total_Collectables');

        // dd($accountReceivablesCollectablesEsrd,$accountReceivablesCollectablesNonEsrd,$accountReceivablesCollectablesTotal);
        ...
        $payerChargesCount = count($payercharges); //array count
        $payerCollections = Arr::pluck($payers, 'collectionAmount');
        $payerCollectionsCount = count($payerCollections); //array count
        ...
        $startDate = \Carbon\Carbon::parse($max)->firstOfMonth()->subMonth(11)->format('m/d/Y');
        ...
        //cpt code analysis
        // name of proc is is monthly but it calculates yearly
        $cptCode = DB::select('CALL monthly_cpdcode_analysis(?,?,?,?,?,?,?)', [$practice_id, '', '', '', '', $startDate, $currentDate]);
        // dd($cptCode,$currentMonth, $currentDate);

        $cptCodeLabeles = Arr::pluck($cptCode, 'cptcode');
        $cptCodeUnits = Arr::pluck($cptCode, 'units');

        $cptCodeRevenue = DB::select('CALL monthly_cpdcode_analysis_revenue(?,?,?,?,?,?,?)', [$practice_id, '', '', '', '', $startDate, $currentDate]);
        // dd($cptCodeRevenue);
        $cptCodeRevenueLabeles = Arr::pluck($cptCodeRevenue, 'cptcode');
        $cptCodeRevenueUnits = Arr::pluck($cptCodeRevenue, 'revenue');

        // dd($averagePayerCharges, $averagePayerCollections);
        return view('backend.practice-dashboard', compact(
            'newPatientAnalysisLabeles',
            'newPatientAnalysisTargets',
            'newPatientAnalysisAchieves',
            'actualCollectionMix',
            'actualCollectionMax',
            'accountReceivablesOpenBalancesEsrd',
            'accountReceivablesOpenBalancesNonEsrd',
            'accountReceivablesOpenBalancesTotal',
            'accountReceivablesCollectablesEsrd',
            'accountReceivablesCollectablesNonEsrd',
            'accountReceivablesCollectablesTotal',
            'accountReceivablesStats',
            // 'accountReceivables',
            'collectionAndTargetCurrentMonth',
            'collectionAndTargetCurrentYear',
            'projectedCollections',
            'actualCollections',
            'patientVolume',
            'projectedCollectables',
            'revenues',
            'cptCodeLabeles',
            'cptCodeUnits',
            'cptCodeRevenueLabeles',
            'cptCodeRevenueUnits',
            'projectedMonthLables',
            // 'accountReceivablesOpenBalances',
            // 'accountReceivablesCollectable',
            'projectedActualCollections',
            'providers',
            'locations',
            'payerLabels',
            'payercharges',
    ...
    public function search(Request $request)
    {
        try {
            ...
            $projectedActualCollections = DB::select("CALL charge_payment_analysis($practice_id,'$location','$provider','$payer','')");
            $projectedMonthLables = Arr::pluck($projectedActualCollection, 'MONTH');
            $actualCollections = Arr::pluck($projectedActualCollection, 'Actual_Collection');
            $projectedCollections = Arr::pluck($projectedActualCollection, 'Projected_Collection');
            $actualCollectionMix = (!empty($actualCollections)) ? normalPrettyPrice2(min(($actualCollections))) : 0;
...
// dd($account_receivables_collectables);
        ...
        $accountReceivablesCollectablesTotal = Arr::pluck($account_receivables_collectables, 'Total_Collectables');
            ...
            $cptCodeRevenue = DB::select('CALL monthly_cpdcode_analysis_revenue(?,?,?,?,?,?,?)', [$practice_id, '$location', '$provider', '$payer', '', $currentMonth, $currentDate]);
            $cptCodeRevenueLabeles = Arr::pluck($cptCodeRevenue, 'cptcode');
            $cptCodeRevenueUnits = Arr::pluck($cptCodeRevenue, 'revenue');
            
            $view = (string)view('backend.partials.practiceDashboardGraphBody', compact(
                'newPatientAnalysisLabeles',
                'newPatientAnalysisTargets',
                'newPatientAnalysisAchieves',
                'actualCollectionMix',
            ...
            $stat = (string)view('backend.partials.statsRow', compact(
                'activePatientStats',
                'newPatientStats',
                'esrdStats',
                'nonEsrdStats'
