...
trait UserAttribute
{
    ...
    public function getChangePasswordButtonAttribute($class)
    {
        ...
        if (access()->user()->id != $this->id && access()->allow('edit-user')) {
            return '<a class="'.$class.'" href="'.route('admin.access.user.change-password', $this).'">
                       <i style="padding-right:5px;" class="fa fa-refresh" data-toggle="tooltip" data-placement="top" title="'.trans('buttons.backend.access.users.change_password').'">
                        </i>Change pass
                    </a>';
        }
    }
            ...
            switch ($this->status) {
                case 0:
                    if (access()->allow('activate-user')) {
                        $name = $class == '' ? 'Activate' : 'btn btn-default btn-flat';


                        return '<a class="'.$class.'" href="'.route('admin.access.user.mark', [$this, 1]).'"><i style="padding-right:5px;" class="fa fa-check-square" data-toggle="tooltip" data-placement="top" title="'.trans('buttons.backend.access.users.activate').'"></i>'.$name.'</a>';
                    }
                ...
                case 1:
                    if (access()->allow('deactivate-user')) {
                        $name = ($class == '') ? 'Deactivate' : 'btn btn-default btn-flat';


                        return '<a class="'.$class.' btn btn-default btn-flat" href="'.route('admin.access.user.mark', [$this, 0]).'"><i style="padding-right:5px;" class="fa fa-square" data-toggle="tooltip" data-placement="top" title="'.trans('buttons.backend.access.users.deactivate').'"></i>'.$name.'</a>';
                    }
    ...
    public function getConfirmedButtonAttribute($class)
    {
        if (!$this->isConfirmed() && access()->allow('edit-user')) {
            return '<a class="'.$class.' btn btn-default btn-flat" href="'.route('admin.access.user.account.confirm.resend', $this).'"><i style="padding-right:5px;" class="fa fa-reload" data-toggle="tooltip" data-placement="top" title='.trans('buttons.backend.access.users.resend_email').'"></i></a> ';
        }
    ...
    public function getDeleteButtonAttribute($class)
    {
        if ($this->id != access()->id() && access()->allow('delete-user')) {
            ...
            return '<a class="'.$class.' btn btn-default btn-flat" href="'.route('admin.access.user.destroy', $this).'"
                 data-method="delete"
